...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        cursor.execute("""
            select type,permalink 
            from core_object 
            where type in (1,2) 
            group by type, permalink having count(*)>1""")
        for type, permalink in cursor.fetchall():
            objs = list(orm.Object.objects.filter(
                    type=type,
                    permalink=permalink).order_by('id'))
            print objs[0].permalink, objs[0].type
            for n, obj in enumerate(objs[1:]):
                obj.permalink = "%s-%s" % (objs[0].permalink, n+1)
                obj.save()
        
        db.execute("""create unique index "one_permalink_per_film_or_person" on "core_object" (type, permalink) where type in (1,2)""")


    def backwards(self, orm):
        "Write your backwards methods here."
        db.execute("""drop index "one_permalink_per_film_or_person" """)
