...
def _get_next(request, comment_id=None):
    """
...
def free_comment(request, content_type=None, object_id=None, edit_id=None, parent_id=None, add_messages=False, ajax=False, model=FreeThreadedComment, form_class=FreeThreadedCommentForm, context_processors=[], extra_context={}):
    ...
    if form.is_valid():
        ...
        new_comment.permalink = "COMMENT"
        new_comment.status = ThreadedComment.PUBLIC_STATUS
        new_comment.type = ThreadedComment.TYPE_COMMENT
        new_comment.save()
        if model == ThreadedComment:
            if add_messages:
        ...
        if ajax == 'json':
            ...
            return HttpResponseRedirect(_get_next(request, new_comment.id))
    elif ajax=="json":
