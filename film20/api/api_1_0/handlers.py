...
class AnonFilmHandler(AnonymousBaseHandler):
    ...
    def auth_guess_rating(cls, film):
        ...
        if request.user.is_authenticated():
            ...
            return recommendations.engine.compute_guess_score(user, film.pk)
...
class AnonUserFilmMetaHandler(AnonymousBaseHandler):
    ...
    def read(self, request, username, permalink):
        ...
        if date:
            ...
            film_channels = [FilmScreeningsChannel(channel=c, screenings=c.screenings) for c in screening_set.get_channels(film.id or film.title)]
        else:
            film_channels = None
