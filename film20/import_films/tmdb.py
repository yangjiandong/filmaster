...
TMDB_API_KEY = getattr(settings, "TMDB_API_KEY", "5ba39e81c0a9248add62049a864c3838")
...
config['urls']['person.search'] = "http://api.themoviedb.org/2.1/Person.search/en/xml/%(apikey)s/%%s" % (config)
...
class MovieDb:
    ...
    def _parsePerson(self, person_element):
        cur_person = {}
        cur_images = ImagesList()
        for item in person_element.getchildren():
            if item.tag.lower() == "images":
                ...
                cur_person[item.tag] = item.text
        cur_person['images'] = cur_images
        return cur_person
        ...
        for item in movie_element.getchildren():
            if item.tag.lower() == "categories":
                ...
                for subitem in item.getchildren():
                    ...
                    size = subitem.get('size').lower()
                    if type == 'poster' and size == 'original':
                        cur_images.set(subitem)
    ...
    def person_search(self, name):
        """Searches for a person by its name.
        Returns SearchResults (a list) containing all matches (Person instances)
        """
        name = urllib.quote(name.encode("utf-8"))
        url = config['urls']['person.search'] % (name)
        etree = XmlHandler(url).getEt()
        search_results = SearchResults()
        for cur_result in etree.find("people").findall("person"):
            cur_person = self._parsePerson(cur_result)
            if cur_person['images']:
                search_results.append(cur_person)
        return search_results
...
def person_search(name):
    """Convenience wrapper for MovieDb.search - so you can do..
    >>> import tmdb
    >>> tmdb.person_search("Julia Roberts")
    """
    mdb = MovieDb()
    return mdb.person_search(name)
