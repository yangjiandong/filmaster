...
class Migration(DataMigration):
    MAP = {
        Object.TYPE_POST: "post",
        Object.TYPE_SHORT_REVIEW: "short_review",
        Object.TYPE_LINK: "link",
    }
    MAP2 = {
        'post': 'post',
    ...
    def forwards(self, orm):
        ...
        query = orm.Watching.objects.filter(object__type__in=self.MAP.keys()).select_related('object')
        total = query.count()
        for i, watching in enumerate(query):
            field = self.MAP[watching.object.type]
            print i, '/', total, field, watching.object_id
            try:
                watching.activity = orm.UserActivity.objects.get(**{field:watching.object_id})
                watching.save()
            except orm.UserActivity.DoesNotExist, e:
                print e
        
        query = ThreadedComment.all_objects.filter(content_type__model__in=self.MAP2.keys()).select_related('content_type')
        total = query.count()
        for i, comment in enumerate(query):
            id = comment.object_id
            field = self.MAP2[comment.content_type.model]
            print i, '/', total, field, comment
            if field:
                try:
                    ua = orm.UserActivity.objects.get(**{field:id})
                    comment.content_object = ua
                    comment._save_activity = False
                    comment.save()
                except orm.UserActivity.DoesNotExist, e:
                    print e
