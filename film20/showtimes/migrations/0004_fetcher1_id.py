...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        query = orm.Cinema.objects.filter(fetcher1_id__isnull=False)
        total = query.count()
        for i, c in enumerate(query):
            print i+1, c.id, c.name, c.fetcher1_id
            with transaction():
                orm.Fetcher.objects.get_or_create(name='google', cid="%s|%s" % (c.fetcher1_id, c.town.country.code), channel=c)
