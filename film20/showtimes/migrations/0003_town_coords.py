...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        logger.info("calculating centers of towns")
        for (i, town) in enumerate(orm.Town.objects.all()):
            logger.debug("%d %r", i, town.name)
            cinemas = list(town.cinema_set.filter(latitude__isnull=False, longitude__isnull=False))
            if len(cinemas):
                town.latitude = sum(c.latitude for c in cinemas)/len(cinemas)
                town.longitude = sum(c.longitude for c in cinemas)/len(cinemas)
                town.save()
